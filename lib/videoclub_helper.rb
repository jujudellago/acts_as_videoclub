  ...
  module Videoclub #:nodoc:
    module Helper #:nodoc:
      ...
      def video_embed_code(video,options={})
        ...
        return video_embed_code_from_url(video.source_url,options)
      end


      def video_embed_code_from_url(source_url, options={})
         ...
         when nil
           return "Error - no source url"
             when /youtu.be/
                #video.source_url =~ /v=([^&]+)/
                #id = $1
                id=source_url.extract_new_youtube_id
         ...
         when /youtube/
           #video.source_url =~ /v=([^&]+)/
           #id = $1
           id=source_url.extract_youtube_id
         ...
         #  id = $1
          id=source_url.extract_vimeo_id 
           if options[:html5]
             <<-EOS
        ...
        when /myspace/
             id=source_url.extract_myspace_id   
              <<-EOS
         ...
         #  id = $1.split("_")[0]
           id=source_url.extract_dailymotion_id
           <<-EOS
